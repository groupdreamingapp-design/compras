...
import { getOpenOrders, getOrderDetails, createReception } from '@/actions/receptionActions';
import { ArrowLeft, CheckCircle, Truck, Thermometer, AlertTriangle, Search, Package } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';

export default function NewReceptionPage() {
    const router = useRouter();
    const [step, setStep] = useState(1); // 1: Select Order, 2: Reception Form
    const [orders, setOrders] = useState([]);
    const [selectedOrder, setSelectedOrder] = useState(null); // { header, items }
    const [loading, setLoading] = useState(true);

    // Header Form (Simplified - No Chofer/Patente, No Temp)
    ...
    const handleSelectOrder = async (orderId) => {
        ...
        const gridItems = details.items.map(i => ({
            ...i,
            receivedQty: i.qty, // Pre-fill with expected
            rejectedQty: 0,
            reason: '',
            lot: '',
            expiry: '',
    ...
    const handleSubmit = async () => {
        ...
        const payload = {
            ...
            temperatura: null, // Removed field
            // Passing empty strings for removed fields to satisfy backend if needed
            chofer: '',
            patente: '',
            qc: {
                receptionUserId: 1, // Hardcoded for now, should be session user
        ...
        if (res.success) {
            ...
            router.push('/dashboard'); // Or finance
        } else {
            ...
            {step === 2 && selectedOrder && (
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label className="label-dark">Nro. Remito</label>
                            <input
                        ...
                        <div>
                            <label className="label-dark">Fecha Recepción</label>
                            <input
                                type="date"
                                className="input-dark w-full"
                                value={header.fecha}
                        ...
                        <div>
                            <label className="label-dark">Estado Global</label>
                            <select
                                className="input-dark w-full"
                                ...
                                {items.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-slate-700/30 transition">
                                        ...
                                        <td className="p-3 text-center bg-slate-700/30">
                                            <input
                                                type="number"
                                                className="bg-slate-900 border border-slate-600 rounded w-full p-1 text-center text-white focus:border-blue-500"
                                        ...
                                        <td className="p-3">
                                            <div className="flex gap-2">
                                                <input
                                                    type="number"
                                                    placeholder="0"
                                                    className="bg-slate-900 border border-slate-600 rounded w-16 p-1 text-center text-red-300 placeholder-slate-600"
                                                ...
                                                <input
                                                    type="text"
                                                    placeholder="Motivo..."
                                                    className="bg-slate-900 border border-slate-600 rounded flex-1 p-1 text-white placeholder-slate-600"
                                                    value={item.reason}
